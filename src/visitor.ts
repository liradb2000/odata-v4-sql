...
export class SQLLiteral extends Literal {
  ...
  "Edm.String"(value: string) {
    return (
      "'" + decodeURIComponent(value).slice(1, -1).replace(/''/g, "'") + "'"
    );
  }
...
export class Visitor {
  ...
  asPostgreSql() {
    ...
    let rx = new RegExp("\\?", "g");
    let rxKey = new RegExp(/\[([^\]]+)\]/, "g");
    let keys = this.parameters.keys();
    this.originalWhere = this.where;
    this.where = this.where.replace(
      rx,
      () => `$${Number(keys.next().value.substring(1)) + 1}`
    );
    this.where = this.where.replace(rxKey, "$1");
    this.orderby = this.orderby.replace(rxKey, "$1");
  ...
  asType() {
    switch (this.type) {
      ...
      case SQLLang.MySql:
      case SQLLang.PostgreSql:
        return this.asPostgreSql();
      case SQLLang.Oracle:
        return this.asOracleSql();
      default:
        return this;
    }
  }
  ...
  protected VisitODataIdentifier(node: Token, context: any) {
    ...
    this.identifiers.add(node.value.name);
    ...
    switch (method) {
      case "contains":
        ...
        if (this.options.useParameters) {
          ...
          this.where += ` like '%${SQLLiteral.convert(
            params[1].value,
            params[1].raw
          ).slice(1, -1)}%'`;
        break;
